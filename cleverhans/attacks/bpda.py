...
class BPDAL2(Attack):
    ...
    def __init__(self, model, reconstructor, sess, dtypestr='float32', **kwargs):
        """
        ...
        super(BPDAL2, self).__init__(model, sess, dtypestr, **kwargs)
    ...
    def generate(self, x, **kwargs):
        ...
        attack = CWL2(self.sess, self.model, self.reconstructor, self.batch_size, self.confidence,
...
class CWL2(object):
    def __init__(self, sess, model, reconstructor, batch_size, confidence, targeted,
        ...
        recon_img = tf.stop_gradient(self.reconstructor.reconstruct(self.newimg, batch_size=batch_size)[0])
        recon_img = (tf.tanh(recon_img) + 1) / 2 * (clip_max - clip_min) + clip_min
        ...
        self.l2dist = reduce_sum(
                tf.square(recon_img - self.other), list(range(1, len(shape))))
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            print('Successful: {} in {}'.format(sum(upper_bound < 1e9), batch_size))
            print('Distortion: {}'.format(mean))
